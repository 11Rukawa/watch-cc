...
## [1.2.0] - 2026-08-25
### Added
- 新增 `--trace <sessionId前缀>`:输出该会话的"思维链条"——按轮次回放
  用户输入、思考过程(thinking)、工具调用与文本回复。
- 工具调用附一句话摘要;Bash 命令的摘要由内置纯规则引擎生成
  (剥 sudo/env 前缀后按 `&&`/`;`/`|` 拆段,逐段匹配规则表,不联网、不调 LLM)。
- 新增 `--think N`:控制 `--trace` 时 thinking 每段最多显示的字符数(默认 600)。
